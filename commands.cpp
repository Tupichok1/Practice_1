...
void saveToFileDataBase(hashTable dataBase, string filePath) {
    int pkSequence = 0;
    string pathOfCSV = filePath + to_string(schemaJson.numberOfCSV) + ".csv";
    string pkSequencePath = filePath + schemaJson.tableName + "_pk_sequence";
    string tablesName = schemaJson.tableName;
    hashTable tableOfColumns = schemaJson.tableColumns;
    ofstream CSVInput(pathOfCSV);
    ofstream sequenceInput(pkSequencePath);
    for ( int i = 0; i < countOfTuples(dataBase); i ++ ) {
        for ( int j = 0; j < tableOfColumns.sizeOfKey(tablesName); j ++ ) {
            if ( dataBase.sizeOfKey(tableOfColumns.findByIndex(tablesName, j)) <= i ) {
                ...
                CSVInput << dataBase.findByIndex(tableOfColumns.findByIndex(tablesName, j), i) << ",";
                pkSequence += 1;
            }
        }
        CSVInput << '\n';
    }
    sequenceInput << pkSequence - 5;
} 
...
struct selectWhere {
    ...
    void loadFromCSV(hashTable& table, string nameOfTable) {
        table.initTable();
    ...
    void parseLine(stringstream& sstream, string word) {
        while ( sstream >> word ) {
            cleanLine(word);
...
bool deleteElement(hashTable& dataBase, stringstream &sstream) {
    ...
    if ( word != "WHERE" ) {
        ...
        while ( fs::exists(schemaJson.nameOfSchema + "/" + tableName + "/" + to_string( schemaJson.numberOfCSV ) + ".csv") ) {
            ...
            saveToFileDataBase(dataBase, schemaJson.nameOfSchema + "/" + tableName + "/" );
            schemaJson.numberOfCSV += 1;
            dataBase.initTable();
        } 
    }
...
int main() {
    start: string commandLine, command;
    ...
    if ( command == "INSERT" ) {
        ...
        saveToFileDataBase(dataBase, schemaJson.nameOfSchema + "/" + schemaJson.tableName + "/"); 
    }
